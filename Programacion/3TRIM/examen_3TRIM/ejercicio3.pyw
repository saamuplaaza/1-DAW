...
Label(ventana, text="Seleccione una opcion").grid(row=1, column=0)
select = OptionMenu(ventana, opcionMenu, listaImagenes[0], listaImagenes[1], listaImagenes[2])
select.grid(row=2, column=0)


# Botón para ver la imagen
Button(ventana, text="Ver", command=seleccionar).grid(row=3, column=0)

seleccionado = Label(ventana)
seleccionado.config(
    anchor=CENTER
)
seleccionado.grid(row=4, column=0, columnspan=7)
